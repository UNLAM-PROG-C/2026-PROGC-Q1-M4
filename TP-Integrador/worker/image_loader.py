...
class ImageLoader:
  ...
  def prepare_from_bytes(
    image_bytes: bytes, target_size: Tuple[int, int] = DEFAULT_TARGET_SIZE
  ) -> np.ndarray:
  ...
  def prepare_from_file(
    image_path: Union[str, Path], target_size: Tuple[int, int] = DEFAULT_TARGET_SIZE
  ) -> np.ndarray:
  ...
  def _to_uint8(image: np.ndarray) -> np.ndarray:
    """Escala float32 [0,1] a uint8 [0,255]; deja el resto de dtypes sin cambios."""
    if image.dtype == np.float32:
      return (image * MAX_PIXEL_VALUE).astype(np.uint8)
    return image
  ...
  def _resize_image(image: np.ndarray, target_size: Tuple[int, int]) -> np.ndarray:
    ...
    """
    pil_image = Image.fromarray(ImageLoader._to_uint8(image))
    pil_image = pil_image.resize(
      (target_size[WIDTH_INDEX], target_size[HEIGHT_INDEX]), Image.Resampling.LANCZOS
    )
